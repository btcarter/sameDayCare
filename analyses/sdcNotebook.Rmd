...
df.sdc.flattened <- df.sdc.icd9.list %>%
  ...
  mutate(
    ...
    sdc_ec = ifelse(AppointmentLocationCVDisplayCVDSC %in% sdc.ec.facilities, "SDC or EC Facility", "Other Facility")
  )
...
DESC <- compareGroups::compareGroups(
  ~ sdc_ec + ReasonForVisitDSC + GenderDSC + RaceDSC + EthnicityDSC + MaritalStatusDSC,
  df.sdc.flattened, 
  max.xlev = 22
  )
