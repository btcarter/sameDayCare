...
start.rows <- setdiff(names(df), c(geo.rows, return.vars))
...
df.new <- df %>% 
  mutate(
    return_in_14_sdc = if_else(
      return_in_14 == TRUE & return_NurseUnit %in% SDC,
      TRUE,
      FALSE
    ),
    return_types = if_else(
      return_in_14 == TRUE & return_Building %in% SDC,
      "SDC",
      if_else(
        return_in_14 == TRUE & return_Building %in% hospitals,
        "Hospital",
        "No return"
      )
    )
...
ff.comp <- compareGroups(frequentFlier ~  Total_SDC_Visits+return_in_14_sdc+return_in_14+Sex+AdmitAge+Race+Fed_Ethnicity+tribal_affil+RUCC_2013+median_income_2018+median_home_value_2018+dist_eu_pt_sdc+dist_eu_pt_return,
                         df.ff,
                         max.xlev = 55)
...
n.return <- length(d.frame$priority_in_return[d.frame$return_in_14 == TRUE & d.frame$priority_in_return == TRUE])

n.return.per <- round(100*n.return/nrow(d.frame[d.frame$return_in_14 == TRUE,]), 2)

n.sdc <- length(d.frame$priority_in_return[d.frame$return_in_14_sdc == TRUE & d.frame$priority_in_return == TRUE])

n.sdc.per <- round(100*n.sdc/nrow(d.frame[d.frame$return_in_14_sdc == TRUE,]), 2)
