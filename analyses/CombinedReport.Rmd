...
df.name <- file.path(data.dir.path, "sdc.flat2.xlsx")
...
# Returns to hospital(s) 24 hours after and within 14 days of SDC/EC visit with same ICD-10 present in record.
...
Of the `r nrow(df.ml)` complete cases entered into the model, only `r sum(df.ml$return)` were considered to have returned to a facility with an ER or EMS advertised. This is probably why the model only selected one feature.
...
df.x.icdCodes <- df.x %>% 
  ...
  unique() %>% 
  group_by(PersonID) %>% 
  mutate(ICD_all = paste(ICDs, collapse = "; ")) %>% 
  select(-ICDs) %>% 
  unique()

df.x.icdChapters <- df.x %>% 
  select(
    PersonID,
    ICD_block_all
  ) %>% 
  ...
  mutate(
    ICDs = paste(ICD_block_all, collapse = "; ")
  ) %>% 
  ungroup() %>% 
  select(
    -ICD_block_all
  ...
  unique() %>% 
  group_by(PersonID) %>% 
  mutate(ICD_block_all = paste(ICDs, collapse = "; ")) %>% 
  select(-ICDs) %>% 
  unique()
